...
post '/videos' do
  ...
  Videos.create(:title => title, :description => description, :url => url, :genre => genre)
  redirect to '/videos'
end
#search function
...
get '/videos/genre' do
  genre = params["genre"]
  @videos = Video.select(:genre => genre)
  erb :videos
end
...
post '/videos/:id' do
  ...
  description = params["description"]
  title = params["title"]
  video = Video.find(id).update(:genre => genre, :description => description, :title => title).save
  redirect to "/videos"
end
