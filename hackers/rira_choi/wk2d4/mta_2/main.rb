...
get '/processing' do
    #params.inspect
    @input = params # => {"start_stop"=>"stop_name,line_name", "end_stop" => "stop_name,line_name"}
    @user_start = @input["start_stop"]
    @user_end = @input["end_stop"]
    user_start_stop = @user_start.split(",")
    user_end_stop = @user_end.split(",")

    @start_stop = user_start_stop[0]
    @start_line = user_start_stop[1]
    @ending_stop = user_end_stop[0]
    @ending_line = user_end_stop[1]
