...
$lines = {
  "N" => ["Times Square", "34th", "28th(N)", "23rd(N)", "Union Square(N)", "8th(N)"], # Line N in Hash
  "L" => ["8th(L)", "6th", "Union Square(L)", "3rd", "1st"], # Line L in Hash
  "6" => ["Grand Central", "33rd", "28th(6)", "23rd(6)", "Union Square(6)", "Astor Place"] # Line 6 in Hash
}
...
get "/" do
  ...
  unless @s_stop.nil? # Only displays if a starting stop is chosen
    ...
    @s_union = $lines[@s_line].index("Union Square(" + @s_line + ")") # Index number for Exchange
    @f_union = $lines[@f_line].index("Union Square(" + @f_line + ")") # Since Union Square need unique strings for each line, we combine the three strings together
