...
if  dep_line == :q
	...
	if dep_line == arr_line 
		...
		if (index_arr - index_dep) > 0 #check the direction
			...
			puts "Your trip starts from #{mta[dep_line][index_dep]}. The stop stations are: #{user_stops_dep}"
			...
			puts "Your trip starts from #{mta[dep_line][r_index_dep]}. The stop stations are: #{user_stops_dep}"
		end
